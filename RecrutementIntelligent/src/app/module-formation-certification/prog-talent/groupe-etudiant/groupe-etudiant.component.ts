...
export class GroupeEtudiantComponent implements OnInit {
  ...
  // loadEtudiants(): void {
  //   this.utilisateurService.getEtudiants().subscribe((etudiants: Etudiant[]) => {
  //     this.etudiants = etudiants;
  //   });
  // }
  ...
  // diviserEtudiantsEnGroupes(etudiants: Etudiant[], nombreGroupes: number): Etudiant[][] {
  //   const groupes: Etudiant[][] = [];
  //   const tailleGroupe = Math.ceil(etudiants.length / nombreGroupes);

  //   for (let i = 0; i < nombreGroupes; i++) {
  //     groupes[i] = etudiants.slice(i * tailleGroupe, (i + 1) * tailleGroupe);
  //   }
  //   return groupes;
  // }
  ...
  // loadGroupes(): void {
  //   this.utilisateurService.getGroupes().subscribe((groupes: SousGroupe[]) => {
  //     this.groupes = groupes.map(groupe => groupe.etudiants);
  //   });
  // }

  // creerSousGroupes(): void {
  //   if (this.groupForm.valid) {
  //     const nombreGroupes = this.groupForm.value.nombreGroupes;
  //     this.groupes = this.diviserEtudiantsEnGroupes(this.etudiants, nombreGroupes);
  
  //     const groupesToSave: SousGroupe[] = this.groupes.map((groupe, index) => ({
  //       id: index + 1,
  //       etudiants: groupe
  //     }));
  
     
  //     this.utilisateurService.saveGroupes(groupesToSave).subscribe(() => {
